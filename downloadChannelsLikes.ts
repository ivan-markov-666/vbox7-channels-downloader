/**
 * Това е скрипт за сваляне на favorites видеата на цели канали от vbox7.com.
...
// Време за вход в vbox7.com в секунди. Обърнете внимание, че работим с цели секунди, а не с милисекунди.
const loginTimeWait = 120;
...
const downloadChannelsTxtFilePath = process.env.DOWNLOAD_CHANNELS_LIKES_TXT_FILE_PATH;
// Проверка с оператора == null
...
export async function vbox7() {
    ...
    // Навигиране страницата за вход в vbox7
    await navigateAndWaitForPageLoad(driver, 'https://www.vbox7.com/login');
    try {
        await clickElement(driver, acceptCookiesButton);
    } catch (error) {
        informMessage(`Cookies банерът не съществува. Продължаваме напред.`);
    }
    alertMessage(`От този момент нататък имате точно ${120} секунди за да се впишете в vbox7 ръчно!`);
    await wait(loginTimeWait); // Изчакваме определеното време, за да се впишете в vbox7 ръчно
    alertMessage(`Времето изтече!`);

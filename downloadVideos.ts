/**
 * Това е скрипт за сваляне на видеа vbox7.com.
...
const downloadVideosTxtFilePath = process.env.DOWNLOAD_VIDEOS_TXT_FILE_PATH;
// Проверка с оператора == null
if (downloadVideosTxtFilePath == null) {
    throw new Error('Стойността на downloadVideosTxtFilePath е null.');
} else {
    informMessage(`Стойността на downloadFolder е ${downloadVideosTxtFilePath} и е валидна.`);
}

// Проверяваме дали TXT файла съдържащ vbox7 каналите за сваляне съществува, в него трябва да има поне една стойност.
const fileExists = doesFileExist(downloadVideosTxtFilePath);
if (!fileExists) {
    throw new Error(`Файлът ${downloadVideosTxtFilePath} НЕ съществува. Създайте го и добавете vbox7 каналите за сваляне в него.`);
}
...
// Дефиниране на пътя на файла който съдържа списък с URL адреси на vbox7 видеата за сваляне
const videosInArray = readLinesFromFile(downloadVideosTxtFilePath);

// Това е основния метод в този файл. Той отговаря за извикването на всички останали методи и за изпълнението на основната логика на скрипта за сваляне на видео файлове от vbox7.
...
export async function vbox7() {
    ...
    // Започва се...
    try {
        // Въртим толкова пъти, колкото видеа сме вкарали в txt файла
        for (let videoIndex = 0; videoIndex < videosInArray.length; videoIndex++) {
            // Вземаме URL адреса на канала
            const vbox7VideoUrl = videosInArray[videoIndex];
            informMessage(`URL на видеото което ще сваляме е: ${vbox7VideoUrl}`);
            await navigateAndWaitForPageLoad(driver, vbox7VideoUrl);
            try {
                await clickElement(driver, acceptCookiesButton);
            } catch (error) {
                // It is OK if the button is not present on the page
            }
            ...
            // const currentPageUrl = `${vbox7VideoUrl}?page=${pageIndex}`;
            ...
            while (videoRetryCount < maxVideoRetries) {
                ...
                // Намиране на елемента съдържащ името на видеото
                const videoNameElement: WebElement = await driver.findElement(By.xpath(videoNameLocator));
                // Get the name of the video.
                const videoName = await getElementText(videoNameElement);
                ...
                const videoId = extractVideoIdFromVideoUrl(vbox7VideoUrl);

                // Извикване на extractMp4Urls за да получите MP4 файловете от страницата
                const mp4Files = await extractMp4Urls(vbox7VideoUrl, sanitizedVideoName, channelName);
                ...
                for (let fileIndex = 0; fileIndex < filteredMp4Files.length; fileIndex++) {
                    ...
                    for (let retry = 0; retry < maxRetries; retry++) {
                        try {
                            ...
                            errorMessage(`Видео файл с име: ${sanitizedVideoName} и URL адрес: ${vbox7VideoUrl} не беше свален!`);
                            errorMessage(`Информацията за това видео е записана в log файла ${logFilePath_videosWasntDownloaded}`);
                            writeToLogFile(logFilePath_videosWasntDownloaded, `Канал: ${channelName}\nВидео файл с име: ${sanitizedVideoName}\nURL адрес на видео файла: ${vbox7VideoUrl}\n\n`);
                            if (retry === maxRetries) {
